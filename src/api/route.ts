...
export const logIn = async(data: SignUpForm):Promise<User> => {
    const info = await api.get(`/users?register=${data.user}&password=${data.pwd}`)
   
   const res:User  = await info.data
...
export const getRequestDocument = async (id: string) => {
    try{
        const res = await api.get(`https://json-documentos.vercel.app/requests/${id}`)
...
export const putStartProcces = async(data:Request) => {
    try{
        const res = await api.put(`/requests/${data.id as string}`,{
...
export const saveInfoMatricula = async(data: Request) => {
    try{
        ...
        if(res.status === 200){
            return {message: "Solicitação Salva com sucesso!"}
        }
    }catch(err){
        console.log(err)
    }
...
export const putHistoric = async(data: Historic) => {
    try{
        const res = await api.put(`/historics/${data.id as string}`,{
            nome_operador: data.nome_operador,
            matricula_operador: data.matricula_operador,
...
export const postHistoric = async(data: Historic) => {
    try{
        const res = await api.post(`/historics`,{
            nome_operador: data.nome_operador,
            matricula_operador: data.matricula_operador,
            data_informacao: data.data_informacao,
            data_correcao:"",
